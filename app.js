...
function showScreen(id, archiveTarget) {
  ...
  if (id === 'archive' && archiveTarget) {
    setTimeout(() => {
      // Find the tag from the element's data-tag, or match by id
      ...
      if (filterBtn) filterArchive(tag, filterBtn);
...
function filterArchive(tag, btn) {
  ...
  // Show / hide exhibit cases
  document.querySelectorAll('.exhibit-case').forEach(card => {
