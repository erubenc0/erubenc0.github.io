...
function _unitProgress(unitIdx) {
  ...
  if (saved >= unit.topics.length)      return { topicIdx: unit.topics.length, pct: 100 };
  return {
    topicIdx: saved,
    pct: Math.round((saved / unit.topics.length) * 100)
  };
}
...
function renderUnitCards() {
  ...
  UNITS.forEach((unit, i) => {
    ...
    const resumeHtml = isStarted && !isComplete
      ...
      : isComplete ? `<span class="resume-badge">Complete ✓</span>` : '';
...
function openUnit(unitIdx) {
  ...
  const topicsHtml = unit.topics.map((t, i) => {
    ...
    return `
      <div class="unit-topic-row">
        <span class="unit-topic-num">${t.num}</span>
        <span class="unit-topic-name">${t.name}</span>
  ...
  const beginBtn = document.querySelector('#unit-title .proceed-row .btn-primary');
  if (beginBtn) {
...
function _loadTopicContent(unitIdx, topicIdx) {
  ...
  let problems = `
    <div class="problem-wrap">
      <div class="problem-q">Problem 1: [ Problem statement for ${name} ]</div>
...
function _renderStepDots(activeStep) {
  ...
  STEP_DOT_IDS.forEach((dotId, i) => {
    const el = document.getElementById(dotId);
    if (!el) return;
    el.innerHTML = STEP_SCREENS.map((_, j) => {
...
// Render correct dots when each panel becomes active
...
function proceedFromApplication() {
  ...
  if (nextIdx < unit.topics.length) {
    ...
    _saveProgress(currentUnitIdx, nextIdx);
    _showTopicIntro(currentUnitIdx, nextIdx);
  } else {
    _saveProgress(currentUnitIdx, unit.topics.length); // mark complete
    _showUnitComplete();
  }
...
function _showUnitComplete() {
  ...
  document.getElementById('unit-complete-content').innerHTML = `
    ...
    <div class="unit-complete-actions">
      ...
      <button class="btn btn-primary" onclick="showScreen('unit-test')">Take the Unit Test →</button>
...
function submitTest() {
  ...
  _showUnitComplete();
  alert(`Test submitted! ${autoGraded.length > 0 ? `Auto-graded: ${correct}/${autoGraded.length} correct. ` : ''}View full feedback in the Learn → Feedback tab.`);
}
