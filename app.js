...
const ALL_SCREENS = [
  ...
  'references', 'qa', 'about', 'login', 'register', 'profile', 'post-composer', 'user-profile'
];
...
const NAV_MAP = {
  ...
  'profile': 'nav-qa', 'post-composer': 'nav-qa', 'user-profile': 'nav-qa',
  'about': 'nav-about'
};
...
function showScreen(id) {
  ...
  if (id === 'user-profile') { _renderUserProfile(); }
}
...
// ── Milestone tiers ────────────────────────────────────────────
const MILESTONES = [
  { posts: 5,   title: 'Prime',            color: null,   frame: null,   avatarFrame: null,     badge: null },
  { posts: 15,  title: 'Computist',        color: null,   frame: null,   avatarFrame: null,     badge: null },
  { posts: 25,  title: 'Community Mentor', color: 'blue', frame: null,   avatarFrame: null,     badge: null },
  { posts: 40,  title: "Euler's Disciple", color: null,   frame: null,   avatarFrame: null,     badge: 'e'  },
  { posts: 50,  title: 'Gauss Incarnate',  color: 'red',  frame: null,   avatarFrame: null,     badge: null },
  { posts: 75,  title: 'The Proofreader',  color: 'red',  frame: 'red',  avatarFrame: null,     badge: '∞'  },
  { posts: 100, title: 'The Professor',    color: 'gold', frame: 'gold', avatarFrame: 'gold',   badge: null },
];

// ── Secret achievements ────────────────────────────────────────
const SECRET_ACHIEVEMENTS = {
  piParadox:  { label: 'The Pi Paradox', badge: 'π',  avatarFrame: 'circle' },
  allKnowing: { label: 'All Knowing',    badge: '💡', avatarFrame: null     },
  mysterious: { label: 'Mysterious',     badge: '?',  avatarFrame: null     },
  sigma:      { label: 'Sigma',          badge: '😎', avatarFrame: null     },
};

// ── Unlockable text colors (purple is creator-only, not listed here) ──
const COLOR_OPTIONS = {
...
function _bootAuth() {
  ...
  auth.onAuthStateChanged(async user => {
    ...
    if (user) {
      ...
      // Auto-create profile doc if it doesn't exist yet
      // (handles accounts created before this version)
      if (!_currentProfile) {
        _currentProfile = await _ensureProfileDoc(user);
      }
...
// Creates a profile doc for existing Firebase Auth accounts that predate
// this version of the app (no Firestore doc was created at sign-up time).
async function _ensureProfileDoc(user) {
  const db = _getDb();
  if (!db) return null;
  try {
    const isCreator  = _isCreator(user.uid);
    const username   = (user.displayName || user.email?.split('@')[0] || 'User').replace(/\s+/g, '_');
    const profileData = {
      username,
      email:          user.email || '',
      color:          isCreator ? 'purple' : 'white',
      title:          isCreator ? 'Creator' : '',
      equippedBadges: [],
      profileFrame:   null,
      avatarFrame:    null,
      photoURL:       null,
      ...
      replyCount:     0,
      isCreator,
      customCosmetics: isCreator ? ['white','blue','red','gold','purple'] : [],
    ...
    console.warn('Could not create profile doc:', e);
    return null;
  }
}
...
function _getEffectiveColor(profile) {
  if (!profile) return '#f0ebe3';
  if (_isCreator(_currentUser?.uid)) return '#9b59b6';
  const key = profile.color || 'white';
  const FULL_COLORS = { ...COLOR_OPTIONS, purple: { value: '#9b59b6' } };
  return FULL_COLORS[key]?.value || '#f0ebe3';
}
...
function _canUseColor(colorKey) {
  if (!_currentUser) return false;
  if (_isCreator(_currentUser.uid)) return true;
      ...
      .then(async cred => {
        ...
        if (db) {
          ...
          await setDoc(doc(db, 'users', cred.user.uid), {
            username, email, color: isCreator ? 'purple' : 'white',
            title: isCreator ? 'Creator' : '',
            equippedBadges: [], profileFrame: null, avatarFrame: null,
            photoURL: null, postCount: 0, replyCount: 0,
            isCreator, customCosmetics: isCreator ? ['white','blue','red','gold','purple'] : [],
            achievements: { piParadox: false, allKnowing: false, mysterious: false, sigma: false }
          });
          // Reserve username
...
function _refreshQaAuthBar() {
  ...
  if (_currentUser) {
    ...
    const titleStr  = isCreator
      ? '<span class="qa-tag-creator">Creator</span>'
      : (milestone ? `<span class="qa-tag-title">${milestone.title}</span>` : '');
    const displayName = profile.username || _currentUser.displayName || _currentUser.email || 'User';
    label.innerHTML = `<span style="color:${color};font-weight:500;">${_escHtml(displayName)}</span> ${titleStr}`;
...
function _buildThread(id, data, section) {
  ...
  const FULL_COLORS   = { ...COLOR_OPTIONS, purple: { value: '#9b59b6' } };
  const authorColor   = isCreatorPost ? '#9b59b6' : (FULL_COLORS[data.authorColor]?.value || FULL_COLORS.white.value);
  ...
  const deleteBtn = canDelete
    ? `<button class="qa-action-btn qa-delete-btn" onclick="_deletePost('${id}','${section}')">Delete</button>`
    : '';

  const upvoteCount = (data.upvotes || []).length;
  const hasUpvoted  = _currentUser && (data.upvotes || []).includes(_currentUser.uid);

  // Fake avatar from authorPhotoURL if stored, else initials
  const avatarProfile = { username: data.authorName, photoURL: data.authorPhotoURL || null, avatarFrame: data.authorAvatarFrame || 'square' };
  const avatarHtml    = _buildAvatarHtml(avatarProfile, authorColor, 38);
  ...
  const replyArea = _currentUser
    ? `<div class="qa-reply-compose">
         <textarea class="qa-reply-input" id="reply-input-${id}" placeholder="Write a reply…" rows="2"></textarea>
         <div class="qa-reply-compose-actions">
           <button class="btn btn-primary qa-btn-sm" onclick="_submitReply('${id}','${section}')">Reply</button>
         </div>
       </div>`
    : `<div class="qa-sign-in-prompt"><a onclick="_setAuthBack();showScreen('login')">Sign in to reply</a></div>`;

  div.innerHTML = `
    <div class="qa-thread-inner">
      <!-- Vote column -->
      <div class="qa-vote-col">
        <button class="qa-vote-btn ${hasUpvoted ? 'qa-voted' : ''}" onclick="_toggleUpvote('${id}','${section}')">▲</button>
        <span class="qa-vote-count">${upvoteCount}</span>
      </div>
      <!-- Main content -->
      <div class="qa-thread-main">
        <div class="qa-thread-meta-row">
          ${avatarHtml}
          <span class="qa-author-name" style="color:${authorColor};cursor:pointer;" onclick="_viewUserProfile('${data.authorUid}')">${_escHtml(data.authorName || 'Unknown')}</span>
          ${creatorTag}${titleTag}${badges}
          <span class="qa-thread-datetime">${dt}</span>
          <div class="qa-thread-actions">${deleteBtn}</div>
        </div>
        <div class="qa-thread-title">${_escHtml(data.title)}</div>
        <div class="qa-thread-body">${_escHtml(data.body)}</div>
        <div class="qa-replies-list">${repliesHtml}</div>
        ${replyArea}
      </div>
    </div>
  `;
...
function _buildReplyHtml(r, postId, section) {
  ...
  const FULL_COLORS    = { ...COLOR_OPTIONS, purple: { value: '#9b59b6' } };
  const color          = isCreatorReply ? '#9b59b6' : (FULL_COLORS[r.authorColor]?.value || FULL_COLORS.white.value);
  ...
  const delBtn         = canDel
    ? `<button class="qa-action-btn qa-delete-btn" onclick="_deleteReply('${postId}','${section}',this)" data-uid="${_escHtml(r.authorUid)}" data-body="${_escHtml(r.body)}">Delete</button>`
    : '';

  const avatarProfile = { username: r.authorName, photoURL: r.authorPhotoURL || null, avatarFrame: r.authorAvatarFrame || 'square' };
  const avatarHtml    = _buildAvatarHtml(avatarProfile, color, 28);

  return `
    <div class="qa-reply">
      <div class="qa-reply-vote-col"><button class="qa-vote-btn qa-vote-btn-sm">▲</button></div>
      <div class="qa-reply-main">
        <div class="qa-reply-meta-row">
          ${avatarHtml}
          <span class="qa-author-name qa-author-sm" style="color:${color};cursor:pointer;" onclick="_viewUserProfile('${_escHtml(r.authorUid)}')">${_escHtml(r.authorName || 'Unknown')}</span>
          ${creatorTag}${titleTag}${badges}
...
function _submitPost() {
  ...
  import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js').then(async ({ collection, addDoc, serverTimestamp, doc, setDoc, increment }) => {
    const profile   = _currentProfile || {};
    const isCreator = _isCreator(_currentUser.uid);
    ...
    await addDoc(collection(db, _composerSection), {
      ...
      authorTitle:      isCreator ? 'Creator' : (milestone?.title || profile.title || ''),
      authorBadges:     profile.equippedBadges || [],
      authorPhotoURL:   profile.photoURL || null,
      authorAvatarFrame: profile.avatarFrame || 'square',
    ...
    // Build profile update — use setDoc+merge so it works even if doc is missing
    const newPostCount = (profile.postCount || 0) + 1;
    const updates = { postCount: newPostCount };
    ...
    if (milestone?.color && !isCreator) { updates.color = milestone.color; }
    ...
    await setDoc(doc(db, 'users', _currentUser.uid), updates, { merge: true });
...
function _submitReply(docId, section) {
  ...
  import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js').then(async ({ doc, updateDoc, arrayUnion, setDoc }) => {
    ...
    const newReplyCount = (profile.replyCount || 0) + 1;
    await setDoc(doc(db, 'users', _currentUser.uid), { replyCount: newReplyCount }, { merge: true });
    if (hasSigma && !profile.achievements?.sigma) {
      await setDoc(doc(db, 'users', _currentUser.uid), { 'achievements.sigma': true }, { merge: true });
      _currentProfile = await _loadProfile(_currentUser.uid);
    }
    if (input) input.value = '';
  }).catch(console.error);
...
function _deletePost(docId, section) {
  ...
  const db = _getDb();
  if (!db) return;
  import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js').then(async ({ doc, getDoc, deleteDoc, setDoc }) => {
    // Get the post to find the author
    const postRef  = doc(db, section, docId);
    ...
    if (authorUid) {
      ...
      if (userSnap.exists()) {
        const current = userSnap.data().postCount || 0;
        await setDoc(userRef, { postCount: Math.max(0, current - 1) }, { merge: true });
        if (authorUid === _currentUser.uid) {
          _currentProfile = await _loadProfile(_currentUser.uid);
        }
      }
    }
...
/* ─────────────────────────────────────────────────────────────
   ...
   Add this to your Firebase init script in index.html:
     import { getStorage } from 'https://www.gstatic.com/firebasejs/10.12.0/firebase-storage.js';
     window._firebaseStorage = getStorage(app);
───────────────────────────────────────────────────────────── */
function _getStorage() { return window._firebaseStorage || null; }

async function _uploadAvatar(file) {
  const storage = _getStorage();
  const db      = _getDb();
  if (!storage || !_currentUser) {
    alert('Storage not configured. Add Firebase Storage to your init script in index.html.');
    return;
  }
  ...
  const storageRef = ref(storage, `avatars/${_currentUser.uid}`);
  await uploadBytes(storageRef, file);
  const url = await getDownloadURL(storageRef);
  await setDoc(doc(db, 'users', _currentUser.uid), { photoURL: url }, { merge: true });
  _currentProfile = await _loadProfile(_currentUser.uid);
  _renderProfile();
}
...
async function _viewUserProfile(uid) {
  if (!uid) return;
  if (uid === _currentUser?.uid) { showScreen('profile'); return; }
  _viewingUid = uid;
  showScreen('user-profile');
}

async function _renderUserProfile() {
  const el = document.getElementById('user-profile-content');
  if (!el || !_viewingUid) return;
  el.innerHTML = `<div class="qa-loading">Loading…</div>`;

  const profile = await _loadProfile(_viewingUid);
  if (!profile) { el.innerHTML = `<div class="qa-error">User not found.</div>`; return; }

  const isCreator  = _isCreator(_viewingUid);
  const color      = isCreator ? '#9b59b6' : (COLOR_OPTIONS[profile.color]?.value || '#f0ebe3');
  const postCount  = profile.postCount  || 0;
  const replyCount = profile.replyCount || 0;
  const milestone  = _computeUnlockedMilestone(postCount);
  const avatarHtml = _buildAvatarHtml(profile, color, 64);

  const milestonesHtml = MILESTONES.map(m => {
    const unlocked = postCount >= m.posts || isCreator;
    return `<div class="profile-milestone ${unlocked ? 'unlocked' : 'locked'}">
  ...
  const secretHtml = Object.entries(SECRET_ACHIEVEMENTS).map(([key, ach]) => {
    ...
    return `<div class="profile-achievement ${earned ? 'unlocked' : 'locked'}">
      <div class="profile-achievement-badge">${earned ? _escHtml(ach.badge) : '?'}</div>
      <div class="profile-achievement-label">${earned ? _escHtml(ach.label) : 'Secret'}</div>
    </div>`;
  }).join('');

  const badgesHtml = (profile.equippedBadges || []).map(b => `<span class="qa-badge">${_escHtml(b)}</span>`).join('') || '<span style="color:var(--text-muted);font-size:.75rem;">No badges yet</span>';
    ...
    <div class="profile-wrap">
      <div class="profile-header">
        ...
        <div class="profile-header-info">
          <div class="profile-username" style="color:${color};">
            ${_escHtml(profile.username || 'User')}
            ${isCreator ? `<span class="qa-tag-creator">Creator</span>` : ''}
            ${milestone  ? `<span class="qa-tag-title">${_escHtml(milestone.title)}</span>` : ''}
          </div>
          <div class="profile-meta">${postCount} posts · ${replyCount} replies</div>
...
/* ─────────────────────────────────────────────────────────────
   AVATAR HTML BUILDER
───────────────────────────────────────────────────────────── */
function _buildAvatarHtml(profile, color, size = 40) {
  const frame     = profile.avatarFrame || 'square';
  const frameStyle = frame === 'circle'   ? 'border-radius:50%;'
                   : frame === 'gold'     ? 'border-radius:6px;box-shadow:0 0 0 2px #c9a84c;'
                   : frame === 'red'      ? 'border-radius:6px;box-shadow:0 0 0 2px #c0504d;'
                   :                       'border-radius:6px;'; // default square
  const sizeStyle = `width:${size}px;height:${size}px;`;
  const initial   = (_escHtml(profile.username || '?'))[0].toUpperCase();

  if (profile.photoURL) {
    return `<img src="${_escHtml(profile.photoURL)}" class="qa-avatar-img"
      style="${sizeStyle}${frameStyle}object-fit:cover;border:2px solid ${color};" alt="avatar">`;
  }
  return `<div class="qa-avatar-initials"
    style="${sizeStyle}${frameStyle}background:var(--card);border:2px solid ${color};
    color:${color};font-size:${Math.round(size*0.4)}px;display:flex;align-items:center;
    justify-content:center;font-family:'EB Garamond',serif;flex-shrink:0;">${initial}</div>`;
}
async function _renderProfile() {
  const el = document.getElementById('profile-content');
  if (!el) return;
  if (!_currentUser) {
    el.innerHTML = `<div class="qa-empty">You must be signed in. <a onclick="showScreen('login')" style="cursor:pointer;color:var(--maroon-light);">Sign in</a></div>`;
    return;
  }
  el.innerHTML = `<div class="qa-loading">Loading profile…</div>`;

  let profile = _currentProfile || await _loadProfile(_currentUser.uid);
  if (!profile) profile = await _ensureProfileDoc(_currentUser);
  if (!profile) { el.innerHTML = `<div class="qa-error">Could not load profile. Please try again.</div>`; return; }
  ...
  const postCount  = profile.postCount  || 0;
  const replyCount = profile.replyCount || 0;
  const FULL_COLORS = { ...COLOR_OPTIONS, ...(isCreator ? { purple: { label: 'Purple', value: '#9b59b6' } } : {}) };
  const color      = _getEffectiveColor(profile);
  const milestone  = _computeUnlockedMilestone(postCount);
  const avatarHtml = _buildAvatarHtml(profile, color, 72);

  // Color picker
  const colorPicker = Object.entries(FULL_COLORS).map(([key, opt]) => {
    const canUse = _canUseColor(key);
  ...
  // Avatar frames available
  const availableFrames = ['square'];
  if (postCount >= 75 || isCreator) availableFrames.push('red');
  if (postCount >= 100 || isCreator) availableFrames.push('gold');
  if (profile.achievements?.piParadox || isCreator) availableFrames.push('circle');
  const frameLabels = { square: 'Square (Default)', circle: 'Circle (Secret)', red: 'Red Frame', gold: 'Gold Frame' };
  const framePicker = availableFrames.map(f =>
    `<button class="profile-frame-btn ${profile.avatarFrame === f || (!profile.avatarFrame && f === 'square') ? 'selected-frame' : ''}"
      onclick="_setFrame('${f}')">${frameLabels[f] || f}</button>`
  ).join('');

  // Badges — all earned badges the user can equip/unequip
  const earnedBadges = [];
  MILESTONES.forEach(m => { if ((postCount >= m.posts || isCreator) && m.badge) earnedBadges.push(m.badge); });
  Object.entries(SECRET_ACHIEVEMENTS).forEach(([key, ach]) => {
    if (profile.achievements?.[key] || isCreator) earnedBadges.push(ach.badge);
  });
  const equipped = profile.equippedBadges || [];
  const badgePanel = earnedBadges.length
    ? earnedBadges.map(b => `
        <button class="profile-badge-btn ${equipped.includes(b) ? 'equipped-badge' : ''}" onclick="_toggleBadge('${_escHtml(b)}')">
          <span class="qa-badge">${b}</span>
          <span class="profile-badge-state">${equipped.includes(b) ? 'Equipped' : 'Equip'}</span>
        </button>`).join('')
    : `<span style="color:var(--text-muted);font-size:.75rem;">No badges earned yet.</span>`;
    ...
    <div class="profile-wrap">
      ...
      <div class="profile-header">
        <div class="profile-avatar-wrap">
          ${avatarHtml}
          <label class="profile-avatar-upload-btn" title="Change profile picture">
            ✎ <input type="file" accept="image/*" style="display:none;" onchange="_uploadAvatar(this.files[0])">
          </label>
        </div>
        <div class="profile-header-info">
          <div class="profile-username" style="color:${color};">
          ...
          <div class="profile-meta">${postCount} posts · ${replyCount} replies</div>
          <div class="profile-badges">${equipped.map(b => `<span class="qa-badge">${_escHtml(b)}</span>`).join('')}</div>
        </div>
      </div>
      ...
      <div class="profile-card">
        ...
        <div class="profile-field-label" style="margin-top:1.4rem;">Comment Color</div>
        <div class="profile-color-row">${colorPicker}</div>
        <div class="profile-field-hint">Colors unlock at milestone post counts.</div>

        <div class="profile-field-label" style="margin-top:1.4rem;">Avatar Frame</div>
        <div class="profile-frame-row">${framePicker}</div>

        <div class="profile-field-label" style="margin-top:1.4rem;">Badges</div>
        <div class="profile-badge-row">${badgePanel}</div>
      </div>
...
async function _setFrame(frame) {
  if (!_currentUser) return;
  const db = _getDb(); if (!db) return;
  const { doc, setDoc } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
  await setDoc(doc(db, 'users', _currentUser.uid), { avatarFrame: frame }, { merge: true });
  _currentProfile = await _loadProfile(_currentUser.uid);
  _renderProfile();
}

async function _toggleBadge(badge) {
  if (!_currentUser) return;
  const db = _getDb(); if (!db) return;
  const equipped = [...(_currentProfile?.equippedBadges || [])];
  const idx = equipped.indexOf(badge);
  if (idx >= 0) equipped.splice(idx, 1);
  else if (equipped.length < 3) equipped.push(badge); // max 3 equipped
  else { alert('You can equip up to 3 badges at once. Unequip one first.'); return; }
  const { doc, setDoc } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
  await setDoc(doc(db, 'users', _currentUser.uid), { equippedBadges: equipped }, { merge: true });
  _currentProfile = await _loadProfile(_currentUser.uid);
  _renderProfile();
}
