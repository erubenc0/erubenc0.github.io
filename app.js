...
async function _ensureProfileDoc(user) {
  ...
  try {
    ...
    const profileData = {
      ...
      postCount:      0,
      isCreator,
      customCosmetics: isCreator ? Object.keys(COLOR_OPTIONS) : [],
...
async function _renderProfile() {
  ...
  if (!el) return;
  if (!_currentUser) {
    el.innerHTML = `<div class="qa-empty">You must be signed in to view your profile. <a onclick="showScreen('login')" style="cursor:pointer;color:var(--maroon-light);">Sign in</a></div>`;
    return;
  }
